...
import SecondaryMarketplaceABI from "@constants/secondary_marketplace_abi.json";
import {
...
import MarketplaceV3ABI from "../constants/marketplace_v3_abi.json";
...
export const getMarketplaceContract = async (chainId) => {
  ...
  const contract = await new window.web3.eth.Contract(
    chainId === 1 || chainId === 4 ? jsonInterface : MarketplaceV3ABI,
    address
  );
...
export const getDigiMaterialV2Contract = (isMainnet) => {
  ...
  const address = getDigiMaterialV2AddressByChainId(
    isMainnet ? "0x89" : "0x13881"
  );
...
export const getContract = async (auctionContractAddress) => {
  const jsonInterface = [
    ...
    },
    {
      ...
      name: "reclaimERC20",
      inputs: [
        { type: "address", name: "_tokenContract", internalType: "address" },
      ],
    },
    {
      type: "function",
    ...
    },
    {
      ...
      name: "updateMinBidIncrement",
      inputs: [
        { type: "uint256", name: "_minBidIncrement", internalType: "uint256" },
      ],
    },
    {
      type: "function",
    ...
    },
    {
      ...
      name: "updatePlatformFee",
      inputs: [
        { type: "uint256", name: "_platformFee", internalType: "uint256" },
      ],
    },
    {
      type: "function",
...
export const getTokenPriceMatic = async () =>
  new Promise(async (resolve) => {
    try {
      ...
      const web3 = new Web3(
        new Web3.providers.HttpProvider("https://rpc-mainnet.matic.network/")
      );
...
export const getSecondaryMarketplaceContract = (chainId) => {
  const address = getSecondaryMarketplaceAddressByChainId(chainId);
  if (!window.web3?.eth) return null;
  const contract = new window.web3.eth.Contract(
    SecondaryMarketplaceABI,
    address
  );
  return contract;
};
