  ...
  useEffect(() => {
    const fetchCollectionGroups = async () => {
      ...
      sortedCollectionGroups.forEach((digitalaxCollectionGroup) => {
        ...
        if (
          digitalaxCollectionGroup.collections.length >
            getPreviewId(digitalaxCollectionGroup.id) &&
          (digitalaxCollectionGroup.collections.length >= 2 ||
            digitalaxCollectionGroup.collections[
              getPreviewId(digitalaxCollectionGroup.id)
            ].id !== "0")
        ) {
          collections.push({
          ...
          digitalaxCollectionGroup.auctions.length >
            getPreviewId(digitalaxCollectionGroup.id) &&
          (digitalaxCollectionGroup.auctions.length >= 2 ||
            digitalaxCollectionGroup.auctions[
              getPreviewId(digitalaxCollectionGroup.id)
            ].id !== "0")
        ) {
          auctions.push({
