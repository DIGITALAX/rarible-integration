...
const Product = ({ pageTitle }) => {
  ...
  useEffect(() => {
    const fetchGarmentV2ByID = async () => {
      ...
      const { digitalaxCollectionGroup } = await getCollectionGroupById(
        chainId,
        15
      );
      const ids = [];
      ...
      if (!parseInt(isAuction)) {
        ...
        if (digitalaxGarmentV2Collection.id) {
          ...
          if (digitalaxGarmentV2Collection.garments[0].tokenUri) {
            const info = await fetchTokenUri(
              digitalaxGarmentV2Collection.garments[0].tokenUri
            );
            for (let i = 1; i <= 4; i += 1) {
          ...
          if (digitalaxGarmentV2Collection.garments[0].children.length) {
            digitalaxGarmentV2Collection.garments[0].children.forEach(
              async (child) => {
